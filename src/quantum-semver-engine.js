...
const SemverUtils = {
  ...
  gt(v1, v2) {
    return Bun.semver.order(v1, v2) === 1;
  },
  lt(v1, v2) {
    return Bun.semver.order(v1, v2) === -1;
  },
  eq(v1, v2) {
    return Bun.semver.order(v1, v2) === 0;
  },
  ...
  diff(v1, v2) {
    ...
    if (p1.prerelease.join(".") !== p2.prerelease.join("."))
      return "prerelease";
...
class QuantumSemverEngine {
  ...
  generateVersionedFilename(component, channel = "stable", options = {}) {
    ...
    const buildId =
      options.buildId ||
      Bun.hash.crc32(`${component}-${timestamp}`).toString(16).slice(0, 8);
...
if (import.meta.main) {
  ...
  if (args.includes("--version") || args.includes("-v")) {
    ...
    for (const info of engine.getChannelInfo()) {
      console.log(
        `  ${info.channel.padEnd(10)} ${info.stability.padEnd(12)} ${info.retention.padEnd(10)} ${info.currentVersion}`,
      );
    }
    ...
    console.log(
      `  Order: ${result.order > 0 ? "v1 > v2" : result.order < 0 ? "v1 < v2" : "equal"}`,
    );
    ...
    console.log(
      `Satisfies ^1.3.0: ${Bun.semver.satisfies(testVersion, "^1.3.0")}`,
    );
    ...
    console.log(
      `Increment prerelease: ${SemverUtils.inc(testVersion, "prerelease", "beta")}`,
    );
  } else {
    console.log(`
