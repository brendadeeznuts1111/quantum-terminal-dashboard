...
function drawFooter() {
  ...
  const activeCount = tradingBots.filter(
    (b) => b.state === "ACTIVE" || b.state === "TRADING",
  ).length;
  ...
  console.log();
  console.log(
    `${ANSI.dim}F1: Help  F2: Add Bot  F3: Kill Bot  F5: Refresh  F10: Quit  ${ANSI.yellow}Ctrl+C: Exit${ANSI.reset}`,
  );
}
...
function start() {
  ...
  console.log(
    `${ANSI.dim}Monitoring ${tradingBots.length} trading bots${ANSI.reset}`,
  );
