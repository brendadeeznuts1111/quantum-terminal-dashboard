...
function generateSparkline(history, width = 10) {
  ...
  return history.slice(-width).map(val => {
...
function drawHeader() {
  ...
  console.log(`${COLORS.cyan}${COLORS.bright}  QUANTUM FINANCIAL TICKER${COLORS.reset}${' '.repeat(30)}${COLORS.dim}${date} ${time}${COLORS.reset}`);
  console.log(`${COLORS.cyan}${COLORS.bright}${'='.repeat(80)}${COLORS.reset}`);
  console.log();
}
...
function drawStockRow(symbol, data) {
  ...
  const row = [
    ...
    CONFIG.showVolume ? `${COLORS.dim}${padStart(formatVolume(volume), 10)}${COLORS.reset}` : '',
    CONFIG.showChart ? `${changeColor}${generateSparkline(history, 10)}${COLORS.reset}` : ''
  ].filter(Boolean).join(' | ');
 ...
 */
function start() {
  console.log(`${COLORS.cyan}Starting Quantum Financial Ticker...${COLORS.reset}`);
  console.log(`${COLORS.dim}Symbols: ${CONFIG.symbols.join(', ')}${COLORS.reset}`);
  console.log(`${COLORS.dim}Update interval: ${CONFIG.updateInterval}ms${COLORS.reset}`);
  console.log();
