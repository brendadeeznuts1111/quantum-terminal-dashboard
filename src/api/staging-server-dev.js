// [DOMAIN][API][STAGING-DEV][HSL:180,70%,85%][META:{STAGING-API-DEV}][CLASS:StagingAPIServerDev]{BUN-API}

/**
 * Staging API Server (Development Mode)
 * Demonstrates the staging API configuration with real endpoints on development port
  ...
  async handleHealth(req, corsHeaders) {
    ...
    const healthData = {
      ...
      server: {
        ...
        original_port: serverConfig.port,
      },
      quantum: {
...
const stagingServerDev = new StagingAPIServerDev();

// Auto-start if run directly
if (import.meta.main) {
  stagingServerDev.start().catch(console.error);
}

export default stagingServerDev;
export { StagingAPIServerDev };
